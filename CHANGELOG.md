...
- Broad-map contract findings now require strict access on the declared parameter, reject derived, mixed, or nested-pattern map origins and multi-shape dispatch, and identify the exact broad parameter; dual-key findings now group repeated literal fallbacks by function and map origin while one-off compatibility helpers remain evidence and boolean cases coalesce with false-collapse findings.
