defmodule Reach.EvidenceCorpusScan do
  ...
  def run(argv) do
    {opts, args, invalid} =
      OptionParser.parse(argv,
        strict: [kind: :string, limit: :integer],
        aliases: [k: :kind, n: :limit]
      )
    ...
    case args do
      ...
      paths -> scan(paths, kind, limit)
    end
  end

  defp scan(paths, kind, limit) do
    paths
  ...
  defp print_results(results, limit) do
